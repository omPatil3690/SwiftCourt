...
const fullName = process.env.ADMIN_FULL_NAME || "SwiftCourt Admin";
...
async function main() {
  try {
    ...
    if (target) {
      ...
      console.log("Admin user (matched by email) updated:", {
        id: target.id,
        email: target.email,
      });
    } else {
      ...
      if (anyAdmin) {
        ...
        console.log("Existing admin account reassigned to new credentials:", {
          id: target.id,
          email: target.email,
        });
      } else {
